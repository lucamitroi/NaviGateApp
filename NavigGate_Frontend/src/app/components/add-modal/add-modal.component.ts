...
export class AddModalComponent {
  ...
  // Function used to send the post request for the backend. It is used to create the voyage 
  onSubmit(form: NgForm) {
    if (form.valid) {
      this.isLoading.set(true);
      this.voyageAction.sendPostRequest({...form.value, shipId: this.shipId}).subscribe({
        next: (res) => {
