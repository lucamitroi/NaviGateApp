...
public class AuthController : ControllerBase
{
    ...
    public IActionResult Register(UserForRegistrationDto userForRegistration)
    {
        ...
        {
            string sqlCheckUserExists = "SELECT Email FROM NaviGateSchema.Auth WHERE Email = '"
            ...
            if (existingUser.Count() == 0)
            {
                ...
                string sqlAddAuth = @"
                    INSERT INTO NaviGateSchema.Auth (
                        [Email],
                        [PasswordHash],
                        [PasswordSalt]
                ...
                if (_dapper.ExecuteSqlWithParameters(sqlAddAuth, sqlParameters))
                {
                    ...
                    string sqlAddUser = @"
                        INSERT INTO NaviGateSchema.Users(
                            [FirstName],
                            [LastName],
                            [Email]
                        ) VALUES (" +
    ...
    {
        string sqlForHashAndSalt = @"SELECT [PasswordHash], [PasswordSalt] FROM NaviGateSchema.Auth
        ...
        string userIdSql = "SELECT UserId FROM NaviGateSchema.Users WHERE Email = '" + userForLogin.Email + "'";
        string firstNameSql = "SELECT firstName FROM NaviGateSchema.Users WHERE Email = '" + userForLogin.Email + "'";
        string lastNameSql = "SELECT lastName FROM NaviGateSchema.Users WHERE Email = '" + userForLogin.Email + "'";
